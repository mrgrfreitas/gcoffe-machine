...
class Upload
{
    ...
    function __construct($BaseDir = null) {
        ...
        self::$BaseDir = ( (string) $BaseDir ? $BaseDir : FILE['disks']['public']['root']);
    ...
    private function CreateFolder($Folder) {
        if (!file_exists(self::$BaseDir . DIRECTORY_SEPARATOR . $Folder) && !is_dir(self::$BaseDir . DIRECTORY_SEPARATOR . $Folder)):
            mkdir(self::$BaseDir . DIRECTORY_SEPARATOR . $Folder, 0755, true);
        endif;
    }
    ...
    private function UploadImage() {
        ...
        if (!$this->Image):
            ...
            switch ($this->File['type']):
                ...
                case 'image/jpeg':
                case 'image/pjpeg':
                    imagejpeg($NewImage, self::$BaseDir . DIRECTORY_SEPARATOR . $this->Folder  . DIRECTORY_SEPARATOR . $this->Name);
                    break;
                case 'image/png':
                case 'image/x-png':
                    imagepng($NewImage, self::$BaseDir . DIRECTORY_SEPARATOR . $this->Folder  . DIRECTORY_SEPARATOR . $this->Name);
                    break;
            endswitch;
    ...
    private function MoveFile() {
        if (move_uploaded_file($this->File['tmp_name'], self::$BaseDir . DIRECTORY_SEPARATOR . $this->Folder  . DIRECTORY_SEPARATOR . $this->Name)):
            $this->Result = $this->Name;
            $this->Error = null;
        else:
            $this->Result = false;
